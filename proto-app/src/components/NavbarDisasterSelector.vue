  ...
  <div class="wrapper">
    <a @click="toggleMenu">
      <div :class="`open-menu-link ${menuOpen ? 'menu-open' : 'menu-closed'}`">
        <div class="title">
          <div style="display: flex; align-items: center; font-weight:
            500;"><img style="height: 20px; width: 20px; margin-right: 0.5em;" :src="icon"> {{ namePlural }}</div>
...
.open-menu-link.menu-closed:hover {
  background: rgba(variables.$light-grey, 0.3);
}
...
.open-menu-link.menu-open:hover {
  background: rgba(variables.$light-grey, 0.5);
}

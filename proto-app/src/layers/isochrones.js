...
import { flatten, zip } from 'es-toolkit/array';
...
export const isochronesLayer = {
  ...
  addLayer(map, isochronesData, store, beforeId) {
    const id = 'isochrones'
  ...
  updateLayer(map, store) {
    if (!map.getLayer('isochrones-fill')) return;
    const currentTab = store.state.currentTab;
    map.setLayoutProperty('isochrones-fill', 'visibility', currentTab === 'infrastructure' ? 'visible' : 'none');
  },
}
